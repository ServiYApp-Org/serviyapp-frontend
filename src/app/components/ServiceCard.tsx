...
export default function ServiceCard({ id, photo, name, provider, category, duration, rating, price }: { id: number, category: string, photo: string; name: string; provider: string; duration: string; rating: number; price: number }) {
    return (
        <div
                className="flex flex-col w-full max-w-[330px] h-[440px] border border-black/50 rounded-lg hover:scale-105 transition-transform hover:shadow-lg bg-white"
                >
                ...
                <span className="px-2 text-md text-black/40 font-medium">
                    Por {provider}
                </span>
